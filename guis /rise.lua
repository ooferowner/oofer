...
local getcustomassets = {
	['oofer/assets/rise/slice.png'] = 'rbxasset://risesix/slice.png',
	['oofer/assets/rise/blur.png'] = 'rbxasset://risesix/blur.png',
	['oofer/assets/new/blur.png'] = 'rbxassetid://14898786664',
}
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('oofer/assets/new/blur.png')
...
local function downloadFile(path, func)
	if not isfile(path) then
		...
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/7GrandDadPGN/VapeV4ForRoblox/'..readfile('oofer/profiles/commit.txt')..'/'..select(1, path:gsub('oofer/', '')), true)
		end)
		if not suc or res == '404: Not Found' then
			error(res)
		end
		if path:find('.lua') then
			res = '--This watermark is used to delete the file if its cached, remove it to make the file persist after ooferupdates.\n'..res
		end
		writefile(path, res)
	end
...
local function writeFont()
	...
	writefile('oofer/assets/rise/risefont.json', httpService:JSONEncode({
		name = 'ProductSans',
		faces = {
			{style = 'normal', assetId = getcustomasset('oofer/assets/rise/SF-Pro-Rounded-Light.otf'), name = 'Light', weight = 300},
			{style = 'normal', assetId = getcustomasset('oofer/assets/rise/SF-Pro-Rounded-Regular.otf'), name = 'Regular', weight = 400},
			{style = 'normal', assetId = getcustomasset('oofer/assets/rise/SF-Pro-Rounded-Medium.otf'), name = 'Medium', weight = 500},
			{style = 'normal', assetId = getcustomasset('oofer/assets/rise/Icon-1.ttf'), name = 'Icon1', weight = 600},
			{style = 'normal', assetId = getcustomasset('oofer/assets/rise/Icon-3.ttf'), name = 'Icon3', weight = 800}
		}
	}))
	return getcustomasset('oofer/assets/rise/risefont.json')
end
...
do
	...
	local res = isfile('oofer/profiles/color.txt') and loadJson('oofer/profiles/color.txt')
	if res then
				...
				if optionsettings.Profiles then
					...
					if ind then
						if val ~= 'default' then
							...
							if isfile('oofer/profiles/'..val..mainapi.Place..'.txt') and delfile then
								delfile('oofer/profiles/'..val..mainapi.Place..'.txt')
							end
						end
					else
...
function mainapi:Load(skipgui, profile)
	...
	if isfile('oofer/profiles/'..game.GameId..'.gui.txt') then
		guidata = loadJson('oofer/profiles/'..game.GameId..'.gui.txt')
		if not guidata then
			guidata = {Categories = {}}
	...
	if isfile('oofer/profiles/'..self.Profile..self.Place..'.txt') then
		local savedata = loadJson('oofer/profiles/'..self.Profile..self.Place..'.txt')
...
function mainapi:Save(newprofile)
	...
	writefile('oofer/profiles/'..game.GameId..'.gui.txt', httpService:JSONEncode(guidata))
	writefile('oofer/profiles/'..self.Profile..self.Place..'.txt', httpService:JSONEncode(savedata))
end
...
mainapi.Categories.Main:CreateDropdown({
	...
	Function = function(val, mouse)
		if mouse then
			writefile('oofer/profiles/gui.txt', val)
			shared.vapereload = true
			if shared.VapeDeveloper then
				loadstring(readfile('oofer/loader.lua'), 'loader')()
			else
				loadstring(game:HttpGet('https://raw.githubusercontent.com/7GrandDadPGN/VapeV4ForRoblox/'..readfile('oofer/profiles/commit.txt')..'/loader.lua', true))()
			end
		end
	end
})
		...
		for i, v in mainapi.Modules do
			if v.Enabled or table.find(found, i) then
				...
				if interfacebar.Enabled then
					...
					holderline.Image = getcustomasset('oofer/assets/rise/slice.png')
