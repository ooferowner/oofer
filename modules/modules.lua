--This watermark is used to delete the file if its cached, remove it to make the file persist after oofer updates.
...
run(function()
	...
	for _, event in {'PlaceBlockEvent', 'BreakBlockEvent'} do	for _, event in {'PlaceBlockEvent', 'BreakBlockEvent'} do
		bedwars.Client:WaitFor(event):andThen(function(connection)
			oofer:Clean(connection:Connect(function(...)
				ooferEvents[event]:Fire(...)
			end))
	...
	-- Inventory hotbar slot tracking
	oofer:Clean(bedwars.Store.changed:connect(function(new, old)
		if new.Inventory and new.Inventory.hotbarSlot ~= (old.Inventory and old.Inventory.hotbarSlot) then
			store.inventory.hotbarSlot = new.Inventory.hotbarSlot
			ooferEvents.HotbarChanged:Fire(new.Inventory.hotbarSlot)
		end
	end))

	-- Keep tools table in sync
	oofer:Clean(ooferEvents.InventoryAmountChanged.Event:Connect(function()
		store.tools.sword = getSword()
		for _, v in {'stone', 'wood', 'wool'} do
			store.tools[v] = getTool(v)
		end
	...
	-- Example hostile module hook
	oofer:Clean(ooferEvents.EntityDamageEvent.Event:Connect(function(data)
		if data.fromEntity and data.fromEntity ~= lplr.Character then
			-- hostile logic here, e.g., auto‑retaliate or log damage
		end
	end))
end)
